...
//    RESPONSEMSG
    public static final int RESPONSE_MSG = 15;
...
//                    MyToast.show(MainActivity.this, "当前的IP地址是："+IP);
                    startThread1();
                    getDeviceMsg(mDeviceId,"您好");
                    break;
                case IPFALSE:
...
//                    var returnCitySN = {"cip": "112.17.81.117", "cid": "330000", "cname": "浙江省"};
     ...
     * 获取getDeveiceMsg
     */
    public void getDeviceMsg(final String mDeviceId,final String mMsgContent) {
        MsgThr = new Thread() {
            @Override
            public void run() {
                Log.i(TAG,"11111111111111&&&&&"+mDeviceId+"*****"+mMsgContent);
                try {
                    retrofit2.Call<ResponseBody> callBrand1 = ServiceFactory.getMainIns().get_msg(mDeviceId,mMsgContent);
                        ...
                        public void onResponse(retrofit2.Call<ResponseBody> call, Response<ResponseBody> response) {
                            if (response.isSuccessful()) {
                                try {
                                    ...
                                    if (code.equals("200")) {
                                        JSONObject MsgData= jo.getJSONObject("data");
                                        mNickname = MsgData.getString("nickname");
                                        rMsgContent = MsgData.getString("content");
                                        Log.i(TAG, jo.optString("data"));
                                        Log.i(TAG, mNickname +"*****"+rMsgContent);
                                        Message message = new Message();
                    ...
                    Log.i(TAG,"2222222222&&&&&"+mDeviceId+"*****"+mMsgContent);
                } catch (Exception e) {
                    Log.e("提示", "异常信息是：" + e.toString());
                }
            }
        };
        MsgThr.start();
